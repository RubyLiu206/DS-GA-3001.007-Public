...
\title{DS-GA 3001.007: Introduction to Machine Learning 
\newline Homework 3: Ridge and Lasso Regression}
...
In this homework you will investigate regression with $\ell_{2}$ and $\ell_{1}$
...
algorithm''), and projected SGD.
On the theory side you'll {[}optionally{]} derive the largest $\ell_{1}$ regularization
parameter you'll ever need to try. 
...
the training data, along with the target function (i.e. the target function for the square loss function) is shown in Figure
\ref{fig:Training-data-and-target-fn} below.
\begin{figure}
\begin{centering}
\includegraphics[height=2.5in]{training-data-and-target-fn}
\par\end{centering}
...
by looking at \newline \texttt{setup\_problem.py.} The figure can be reproduced
...
include file \texttt{ridge\_regression.py}. You'll go through the following
...
\begin{center}
\includegraphics[width=1\textwidth]{shooting_algo} (Source:
...
\subsection{Optional: Deriving $\lambda_{\mbox{max}}$}

In this problem we will derive an expression for $\lambda_{\text{max}}$.
Use the Lasso objective function excluding
...
\section{Projected SGD via Variable Splitting} 
...
and $\theta^{-}$ to zero.
\begin{enumerate}
\item Implement projected SGD to solve the above optimization
...
\\
\item How does the sparsity compare to the solution from the shooting algorithm? \\
